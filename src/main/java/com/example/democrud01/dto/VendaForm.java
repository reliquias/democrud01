...
public class VendaForm {
	...
	public Venda converterFull(UserService userService, AgenteService agenteService) {
		Optional<UserSistem> usuario = userService.get(idUsuario);
		Optional<Agente> cliente =  agenteService.get(idCliente != null ? idCliente : 0);
		return new Venda(usuario.get(), cliente.isEmpty() ? null : cliente.get(), pagamento.getSaldoDevedor(), pagamento.getDinheiro(), pagamento.getCartaoCredito(), pagamento.getCartaoDebito(), pagamento.getTotal());
    }
